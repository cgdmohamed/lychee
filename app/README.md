# Lychee's Menu — full-stack implementation

A bilingual (EN/AR) digital restaurant menu with an admin CMS, implemented from the
`Lychee Menu.dc.html` Claude Design export in `../project`.
...
1. New Resource → **Docker Compose**, point it at this repo with **Base Directory** set to `app`.
2. Coolify will pick up `app/docker-compose.yml` and `app/Dockerfile` as-is.
